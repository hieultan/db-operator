...
func (r *PostgreSQLReconciler) UpdatePostgreSQLClients(ctx context.Context, pg *postgresv1alpha1.PostgreSQL) (bool, error) {
    log := log.FromContext(ctx).WithName("PostgreSQLReconciler")
    dsn, user, password, err := r.getPostgreSQLConfig(ctx, pg)
    if err != nil {
        return true, err
    }
...
func (r *PostgreSQLReconciler) getPostgreSQLConfig(ctx context.Context, pg *postgresv1alpha1.PostgreSQL) (dsnUser, user, password string, err error) {
